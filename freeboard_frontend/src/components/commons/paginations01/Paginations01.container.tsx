...
export default function Paginations01(props): JSX.Element {
  ...
  const onClickPageButton = (event: MouseEvent<HTMLButtonElement>) => {
    ...
    props.setQqq(Number(event.currentTarget.id)); // setQqq에 페이지 값을 담아서 state 끌어 오르기를 통해서 바로부모한테 전달함
    // 그래서 페이지 번호를 나타낼거임!!
  ...
  const onClickNextPage = () => {
    if (startPage + 10 <= lastPage) {
      ...
      props.setQqq(startPage + 10); // 여기에도 setQqq를 해준 이유는 넥스트페이지버튼은 100페이지 단위로 다음 페이지로 넘어가니 이것도 리패치 같은 느낌을 해줘야함!
  ...
  const onClickPrevPage = () => {
    ...
    props.setQqq(startPage - 10); // nextPage 방식과 같음!
